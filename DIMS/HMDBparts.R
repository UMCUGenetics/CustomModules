...
# define parameters 
parser <- ArgumentParser(description = "HMDBparts")

parser$add_argument("--hmdb_db_file", dest = "hmdb_db_file",
                    help = "File path for the HMDB databse file", required = TRUE)
parser$add_argument("--breaks_file", dest = "breaks_file",
                    help = "File containing all the breaks for binning intensities", required = TRUE)
parser$add_argument("--standard_run", dest = "standard_run",
                    help = "Parameter indicating whether the dataset has m/z between 70 and 610", required = TRUE)
parser$add_argument("--hmdb_parts_path", dest = "hmdb_parts_path",
                    help = "Path to pre-generated HMDB files for standard run", required = TRUE)
...
if (args$standard_run == "yes" & min_mz > 68 & min_mz < 71 & max_mz > 599 & max_mz < 610) {
  # skip generating HMDB parts; copy all files containing hmdb in file name from hmdb_parts_path
  hmdb_parts <- list.files(args$hmdb_parts_path, pattern = "hmdb") 
  for (hmdb_file in hmdb_parts) {
    file.copy(paste(args$hmdb_parts_path, hmdb_file, sep = "/"), "./", recursive = TRUE)
  }
} else { 
