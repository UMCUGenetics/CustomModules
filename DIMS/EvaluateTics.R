...
parser <- ArgumentParser(description = "EvaluateTics")

parser$add_argument("--samplesheet_file", dest = "samplesheet_file",
                    help = "Parsed samplesheet RData file", required = TRUE)
parser$add_argument("--nr_replicates", dest = "nr_replicates",
                    help = "Number of technical replicates per biological sample", required = TRUE)
parser$add_argument("--analysis_id", dest = "analysis_id",
                    help = "Name of the dataset", required = TRUE)
parser$add_argument("--matrix", dest = "matrix",
                    help = "Matrix used (e.g. Plasma, DBS, Research)", required = TRUE)
parser$add_argument("--highest_mz_file", dest = "highest_mz_file",
                    help = "File containing highest m/z value in dataset", required = TRUE)
parser$add_argument("--trim_params_file", dest = "trim_params_file",
                    help = "File containing values of trim parameters", required = TRUE)
...
nr_replicates <- as.numeric(args$nr_replicates)
run_name <- args$analysis_id
dims_matrix <- args$matrix

# load functions
source(paste0(args$preprocessing_scripts_dir, "evaluate_tics_functions.R"))

# get the value for the highest m/z in the dataset
highest_mz <- get(load(args$highest_mz_file))

# load init_file: contains repl_pattern
load(args$samplesheet_file)

# load trim_params_file: contains trim_left_neg, trim_left_pos, trim_right_neg & trim_right_pos
load(args$trim_params_file)

# set the threshold for accepting or rejecting technical replicates
