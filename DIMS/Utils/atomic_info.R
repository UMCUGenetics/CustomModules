## adapted from globalAssignments.HPC.R
# relative abundancies from theoretical mass and composition
# not a function, but a large amount of objects that will become available in memory
# refactor: include in get_element_info
# check if all these elements are necessary. Only use snake_make for Hmass -> hydrogen_mass.
# move source library to higher level
options(digits = 16)
...
# The following list is for our own use
...
# add first isotope of Cl because of high abundance
Cl37 <- list(name = "Cl37", mass = 37, isotope = list(mass = c(-0.03409741)), abundance = c(1))

all_elements <- list(Ba, Br, C, Ca, Cl, Cr, Cu, F, Fe, H, Hg, I, K, Li, Mg, Mn, N, Na, Ni, O, P, S, Se, Si, Sn, Zn)
all_adducts <- list(Ba, Br, Ca, Cl, Cl37, Cr, Cu, Fe, Hg, I, K, Li, Mg, Mn, Na, Ni, Se, Si, Sn, Zn, 
                    NH4, Ac, NaCl, For, Na2, CH3OH, NaCl2, NaCl3, NaCl4, NaCl5, Na3, KCl, H2PO4, HSO4,
                    Met2, Met3, Met4, Met5, NaminH, KminH, H2O, NaK, min2H, plus2H, plus2Na, plusNaH,
                    plusKH, min3H, plus3H, plusHNH, plus3Na, plus2NaH, plusNa2H)

atoms_inuse <- c("P", "O", "N", "C", "H", "S", "Cl", "D", "13C", "34S", "18O", "37Cl")
atomic_weights <- c(30.97376163, 15.99491463, 14.0030740052, 12.0000, 1.0078250321, 31.9720707, 34.968852721, 2.0141017778, 13.0033548378, 33.96786690, 17.9991610, 36.96590259)
electron <- 0.00054858

hydrogen_mass <- Hmass <- H$mass + H$isotope$mass[1]
