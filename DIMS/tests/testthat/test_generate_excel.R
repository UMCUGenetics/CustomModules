...
testthat::test_that("calculate_zscores: Calculating Z-scores using different methods for excluding controls", {
  test_outlist <- read.delim(test_path("fixtures", "test_outlist.txt"))
  test_outlist <- as.data.frame(test_outlist)
  ...
  expect_type(calculate_zscores(test_outlist, "_Zscore", control_col_idx, NULL, intensity_col_ids), "list")
  expect_identical(colnames(calculate_zscores(test_outlist, "_Zscore", control_col_idx, NULL, intensity_col_ids)),
  ...
  expect_equal(round(calculate_zscores(test_outlist, "_Zscore", control_col_idx, NULL, intensity_col_ids)$avg_ctrls, 3),
               c(16129.167, 1150.0, 1231.250, 4015.833), tolerance = 0.001)
  expect_equal(calculate_zscores(test_outlist, "_Zscore", control_col_idx, NULL, intensity_col_ids)$P2.1_Zscore,
               c(-0.2544103, 32.4586955, 13.6066674,  0.4037668), tolerance = 0.001)

  expect_type(calculate_zscores(test_outlist, "_RobustZscore", control_col_idx, perc, intensity_col_ids), "list")
  expect_identical(colnames(calculate_zscores(test_outlist, "_RobustZscore", control_col_idx, perc, intensity_col_ids))[34:47],
  ...
  expect_equal(calculate_zscores(test_outlist, "_RobustZscore", control_col_idx, perc, intensity_col_ids)$avg_ctrls,
               c(1255.0, 1110.0, 1227.5, 2811.5), tolerance = 0.001)
  expect_equal(calculate_zscores(test_outlist, "_RobustZscore", control_col_idx, perc, intensity_col_ids)$P2.1_RobustZscore,
               c(9.1511750, 46.9804468, 16.8039663, 0.8565111), tolerance = 0.001)

  expect_type(calculate_zscores(test_outlist, "_OutlierRemovedZscore", control_col_idx, outlier_threshold, intensity_col_ids), "list")
  expect_identical(colnames(calculate_zscores(test_outlist, "_OutlierRemovedZscore", control_col_idx, outlier_threshold, intensity_col_ids))[34:47],
  ...
  expect_equal(calculate_zscores(test_outlist, "_OutlierRemovedZscore", control_col_idx, outlier_threshold, intensity_col_ids)$avg_ctrls,
               c(1231.818, 1077.273, 1231.250, 2649.091), tolerance = 0.001)
  expect_equal(calculate_zscores(test_outlist, "_OutlierRemovedZscore", control_col_idx, outlier_threshold, intensity_col_ids)$nr_ctrls,
                   c(11, 11, 12, 11))
  expect_equal(calculate_zscores(test_outlist, "_OutlierRemovedZscore", control_col_idx, outlier_threshold, intensity_col_ids)$P2.1_OutlierRemovedZscore,
