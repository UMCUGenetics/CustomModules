...
def get_gender_from_bam(bam, min_mapping_qual, locus_y, ratio_y):
    ...
    with pysam.AlignmentFile(bam, "rb") as bam_file:
        y_reads = float(sum([is_valid_read(read, min_mapping_qual) for read in bam_file.fetch(region=locus_y)]))
...
def compare_and_evaluate_gender(measured_gender, stated_gender):
    ...
    """
    # if gender is unknown/onbekend in database, pass
...
if __name__ == "__main__":
    ...
    parser.add_argument("ratio_y", type=float, help="maximunum chromosome Y ratio for females [float]")
